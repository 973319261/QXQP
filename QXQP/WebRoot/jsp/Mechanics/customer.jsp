...
<body>
   ...
   <!--  @*按钮栏*@ -->
    <div class="cl pd-5 bg-1 bk-gray mt-20" style="padding:10px;margin:0px">
        ...
        <button class="btn btn-info "><i class="Hui-iconfont">&#xe692;</i> 车况单据</button>
   ...
   <!--  @*配件基本资料添加模态框*@ -->
    ...
    <script>
        ...
        function MaintenanceNum() {
            $.post("${cxt}/servlet/CustomerServlet?fun=maintenanceNum", function (data) {
        ...
        function create() {
            ...
            appendOption("RepairID", "${cxt}/servlet/CommonServlet?t=SYS_Repair");//修理类别下拉框
        ...
        {
            $.getJSON("${cxt}/servlet/CustomerServlet?fun=arrlistToAppointment", function (data) {
                data=data.list;
                btnMainInsert(1);
                ...
                $("#Carder").val(text)
                $.post("${cxt}/servlet/CustomerServlet?fun=cleanArrlist");
            }
             layer.close(index);
          })     
        }
        ...
        function btnMainSava() {
            ...
            function Bave() {
                layer.confirm('是否保存当前信息？', {
                    ...
                    var index = layer.load(0); //0代表加载的风格，支持0-2
                    $.post("${cxt}/servlet/CustomerServlet?fun=updateListReceptione", {
                        PW_Reception: JSON.stringify(ArrReception), SYS_RecRepairItemDetail: JSON.stringify(ArrRecRepairItem),
                        SYS_RecProductDetail: JSON.stringify(ArrRecProduct), SYS_RecOtherCostDetail: JSON.stringify(ArrRecOtherCost),
                        ...
                        if (data > 0) {
                            layer.close(index);
                            $.post("${cxt}/servlet/AppointmentServlet?fun=selectToMainten", { PredateID: $("#PredateID").val(), MaintenanceNum: $("#MaintenanceNum").text() });
        ...
        function ToDispatching()
        {
            var ReceptionID = $("#ReceptionID").val();
            if (ReceptionID != 0) {
                $.post("${cxt}/servlet/Dispatching?fun=dispatching&ReceptionID=" + ReceptionID, function (data) {
        ...
        function ReceptionData(record) {
            var index = layer.load(0); //0代表加载的风格，支持0-2
            btnMainInsert(1);
            $.getJSON("${cxt}/servlet/CustomerServlet?fun=selectReceptionDetail&ReceptionID=" + record.receptionID, function (data) {
