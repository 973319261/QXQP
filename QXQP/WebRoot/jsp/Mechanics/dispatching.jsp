...
<body>
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20" style="padding:10px;margin:0px">
        ...
        <button class="btn btn-danger "><i class="Hui-iconfont">&#xe6a6;</i> 关闭</button>
    </div>
  <!--   @*控件栏*@ -->
    ...
    <script>
        ...
        function create() {
            ...
            createSelect("RepairID", "${cxt}/servlet/CommonServlet?t=SYS_Carder");//接车人下拉框
        ...
        {
            $.ajaxSettings.async = false;
            $.getJSON("${cxt}/servlet/CustomerServlet?fun=selectReceptionDetail&ReceptionID=" +ReceptionID, function (data) {
                data=data.list;
                ReceptionDetail(data);
            })
            $.post("${cxt}/servlet/Dispatching?fun=clearDispatching");
            $.getJSON("${cxt}/servlet/Dispatching?fun=selectReception&ReceptionID=" + ReceptionID, function (record) {
        ...
        function ReceptionDetail(data) {
            $.each(data[0], function (i) {
                ...
                var tr = $("<tr><td>" + length + "</td><td><select id=WA" + length + "></select></td><td><select id=WB" + length + " disabled ></select></td>"
                        ...
                        + "<td onclick='tdclick(this)' id=WI" + length + ">" + data[0][i].jobAmount + "</td><td ><input id=WJ" + length + " class='Date' value=" + data[0][i].completionDate + "></td>"
                ...
                $("#WA" + length).val(data[0][i].repairItemID);
                 $.getJSON("${cxt}/servlet/CommonServlet?fun=selectRepairItemChange&RepairItemID=" + data[0][i].repairItemID, function (data) {
                     data=data.list;
                     $("#WB" + length).val(data.maintenanceID);
                })
                        ...
                        $.each($(".xlg"), function (i) {
                            if ($(this).val() == 0) {
                                ...
                                layer.confirm('是否保存当前信息？', {
                                    ...
                                    $.post("${cxt}/servlet/Dispatching?fun=updateListRepairItemDetail", {
                                        SYS_RecRepairItemDetail: JSON.stringify(ArrRecRepairItem), MaintenAmount: $("#MaintenAmount").val(), SelfCoding: SelfCoding, ReceptionID: ReceptionID, ToSendWork: true
                                    }, function (data) {
                                        SeleceReception();
                                        if (data>0) {
        ...
        function ReceptionData(record) {
            ...
            $.getJSON("${cxt}/servlet/CustomerServlet?fun=selectReceptionDetail&ReceptionID=" + record.receptionID, function (datas) {
                datas=datas.list;
