  ...
  <!--   @*按钮栏*@ -->
   ...
   <!--  @*表格*@ -->
    ...
    <script>
        ...
        {
            $.ajaxSettings.async=false;
            $.getJSON("${cxt}/servlet/Collage?fun=selectRestock&ReceptionID=" + $("#ReceptionID").val(), function (data) {
                data=data.list[0];
            ...
            $.getJSON("${cxt}/servlet/Collage?fun=arrlistToRestock", function (data) {
                data=data.list;
                console.log(data)
        ...
        function BaveRestock()
        {
            ...
            if ($("#RestockMan").val() == "") {
                ...
                $.getJSON("${cxt}/servlet/Collage?fun=judgingQuantity",{RestockDetail: JSON.stringify(ArrRestockDetai)},function(data){
                    data=data.list;
                    if (data.state) {
                        $.post("${cxt}/servlet/Collage?fun=updateListRestock", { PW_Restock: JSON.stringify(ArrRestock), RestockDetail: JSON.stringify(ArrRestockDetai) }, function (data) {
                            if (data>0) {
                                layer.msg("退 料 成 功 ！")
                            } else {
                                layer.msg("退 料 失 败 ！")
        ...
        //退料明细
        function Restock(RestockDate,Operator,Amount,Remark,ReceptionID) {
