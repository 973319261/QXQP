    ...
    <script>
        ...
        $(function () {
            ...
            tbDanJu = $.fn.bsgrid.init('tbDanJu', {
                url: '${cxt}/insuranceController/selectInsurance.action',
                autoLoad: false,//自动加载
            ...
            tbDanHao = $.fn.bsgrid.init('tbDanHao', {
                url: '${cxt}/insuranceController/selectDanHao.action',
                autoLoad: false,//自动加载
                stripeRows: true,//隔行变色
                rowHoverColor: true,//划过行变色
                displayBlankRows: false,//是否显示空白行, 默认值true
        ...
        function InsuranceNum() {
            $.post("${cxt}/insuranceController/insuranceNum.action", function (data) {
        ...
        {
            if (ReceptionID != "") {
                $.getJSON("${cxt}/insuranceController/selectInsurances.action?receptionID=" + ReceptionID, function (data) {
                    DanHaoData(data[0]);
                })
            } else {
                SeleceDanJu();
            }
            $.post("${cxt}/insuranceController/clearInsurance.action");
        }
        /********************主页面---开始******************************/
        //删除数据
        function DelectInsurance() {
            ...
            if (InsuranceID != 0) {
                $.post("${cxt}/insuranceController/delectInsurance.action?insuranceID=" + InsuranceID, function (data) {
                    if (data) {
                        layer.msg("删 除 成 功 ！");
                        SeleceDanJu();
                        Clear();
        ...
        function BavaMian() {
            ...
            if (MaintenanceNum == "") {
                ...
                $.post("${cxt}/insuranceController/bavaInsurance.action", { pwInsurance: JSON.stringify(ArrInsurance), sysInsuranceDetail: JSON.stringify(ArrInsuranceDetail) }, function (data) {
                    if (data >0 ) {
                        SeleceDanJu()
        ...
        function TOAudit() {
            ...
            if (InsuranceID != "") {
                $("#ToAudit").val("true");
                $.post("${cxt}/insuranceController/toAudit.action?insuranceID=" + InsuranceID, function (data) {
                    if (data > 0) {
        ...
        function TONotAudit() {
            ...
            $.getJSON("${cxt}/claimsetController/selectBalance.action?businessNum=" + InsuranceNum, function (data) {
                if (data.state && data.states != "A") {
                    $.post("${cxt}/insuranceController/toNotAudit.action?insuranceID=" + $("#InsuranceID").val(), function (data) {
                        if (data > 0) {
        ...
        function MianData(record) {
            Clear();
            $.getJSON("${cxt}/insuranceController/selectReceptionDetail.action?receptionID=" + record.receptionId, function (data) {
        ...
        function DanHaoData(record) {
            ...
            console.log(record)
            $.getJSON("${cxt}/insuranceController/selectInsuranceData.action?insuranceDetailID=" + InsuranceDetailID, function (data) {
                if (data != null) {
                    DanJuData(data[0]);
                } else {
                    MianData(record);
