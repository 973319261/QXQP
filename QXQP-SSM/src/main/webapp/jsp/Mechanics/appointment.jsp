...
<body>
	...
	<script>
        ...
        $(function () {
           ...
           tbKeHu = $.fn.bsgrid.init('tbKeHu', {
            ...
            tbDanJu = $.fn.bsgrid.init('tbDanJu', {
                url: '${cxt}/appointmentController/selectPredate.action',
                autoLoad: false,//自动加载
                stripeRows: true,//隔行变色
                rowHoverColor: true,//划过行变色
                displayBlankRows: false,//是否显示空白行, 默认值true
        ...
        //生成预约单号
        function PredateNum() {
            $.post("${cxt}/appointmentController/predateNum.action", function (data) {
                $("#PredateNum").val(data);
        ...
        function create() {
            ...
            createSelect("RepairID", "${cxt}/commonController/selectAppendOption.action?type=SYS_Repair");//车型下拉框
        ...
        function DetailEvent(length,Num)
        {
            ...
            if (Num == 2) {
                createSelect("CI" + length, "${cxt}/commonController/selectAppendOption.action?type=SYS_Maintainability");
        ...
        function btnMainDelect()
        {
            ...
            if (PredateID != "") {
                $.post("${cxt}/appointmentController/deleteListPredate.action?predateID=" + PredateID, function (data) {
                    if (data) {
        ...
        function btnMainSava() {
            ...
            if (CarNum == "") {
                ...
                layer.confirm('是否保存当前信息？', {
                    ...
                    $.ajax({
			                type: "POST",
			                url: "${cxt}/appointmentController/updateListPredate.action",
        ...
        function ToMainten()
        {
            ...
            if (ToAudit != "false") {
                if (ToTransferOrder == "false") {
                     $.ajax({
			                type: "POST",
			                url: "${cxt}/customerController/customer.action",
        ...
        function Arrlist(ArrPredate, ArrPreRepairItem, ArrPreProduct, ArrPreOtherCost)
        {
            ...
            e.DeserveMoney = $("#Receivable").val();//应收金额
        ...
        function ToAudit() {
            ...
            if (PredateID != "") {
                ...
                $.post("${cxt}/appointmentController/toAudit.action?predateID=" + $("#PredateID").val(), function (data) {
                    if (data) {
        ...
        function ToNotAudit() {
            $("#ToAudit").val("false");
            $.post("${cxt}/appointmentController/toNotAudit.action?predateID=" + $("#PredateID").val(), function (data) {
                if (data) {
        ...
        {
            $.getJSON("${cxt}/commonController/customerNum.action", function (data) {
        ...
        function PredateData(record)
        {
            var index = layer.load(0); //0代表加载的风格，支持0-2
            btnMainInsert(1);
            $.getJSON("${cxt}/appointmentController/selectPredateDetail.action?predateID=" + record.predateId, function (data) {
       ...
       //主页面
        function Predate(PredateId,PredateNum,MaintenanceNum,OpenDate,MaintainData,CarNum,VehicleType,Owner,
            CarMasterPhone, Contacts, Telephone, CarderID, RepairID, Remark, Amount, DeserveMoney, ToAudit, Describe, ToTransferOrder, CustomerNum) {
            ...
            this.deserveMoney = DeserveMoney;//应收金额
