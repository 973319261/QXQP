   ...
   <!--  @*配件基本资料添加模态框*@ -->
    ...
    <script>
        ...
        function create() {
            ...
            createSelect("PSuppliersID", "${cxt}/commonController/selectAppendOption.action?type=SYS_Suppliers");//供应商下拉框
                ...
                $.post("${cxt}/collageController/updateListCollage.action", { pwCollage: JSON.stringify(ArrCollage), sysCollageDetai: JSON.stringify(ArrCollageDetai) }, function (data) {
                    if (data>0) {
                        layer.msg("已减配件库存，领料成功！");
                         $.post("${cxt}/collageController/collage.action?receptionID=" + $("#ReceptionID").val(), function (data) {
