  ...
  <!--   @*控件栏*@ -->
    ...
    <script>
        ...
        function create() {
            ...
            createSelect("RepairID", "${cxt}/commonAction!selectAppendOption.action?type=SYS_Carder");//接车人下拉框
        ...
        {
            $.ajaxSettings.async = false;
            $.getJSON("${cxt}/customerAction!selectReceptionDetail.action?receptionID=" +ReceptionID, function (data) {
                ReceptionDetail(data);
            })
            $.post("${cxt}/dispatchingAction!clearDispatching.action");
            $.getJSON("${cxt}/dispatchingAction!selectReception.action?receptionID=" + ReceptionID, function (record) {
                record=record[0];
        ...
        function ReceptionDetail(data) {
            $.each(data[0], function (i) {
                ...
                $("#WA" + length).val(data[0][i].repairItemId);
                 $.getJSON("${cxt}/commonAction!selectRepairItemChange.action?repairItemID=" + data[0][i].repairItemId, function (data) {
                     $("#WB" + length).val(data.maintenanceId);
                })
                        ...
                        $.each($(".xlg"), function (i) {
                            if ($(this).val() == 0) {
                                ...
                                layer.confirm('是否保存当前信息？', {
                                    ...
                                    $.post("${cxt}/dispatchingAction!updateListRepairItemDetail.action", {
        ...
        function ReceptionData(record) {
            ...
            $.getJSON("${cxt}/customerAction!selectReceptionDetail.action?receptionID=" + record.receptionId, function (datas) {
