                    ...
                    <div style="height:500px;width:950px;overflow:auto; box-shadow: 0 0 20px 5px #ddd;">
                        <table id="tbDanHao" style="width:1520px">
                            <tr>
                                ...
                                <th w_index="insuranceMoney">索赔金额</th>
    ...
    <script>
        ...
        $(function () {
            ...
            tbDanJu = $.fn.bsgrid.init('tbDanJu', {
                url: '${cxt}/insuranceAction!selectInsurance.action',
                autoLoad: false,//自动加载
            ...
            tbDanHao = $.fn.bsgrid.init('tbDanHao', {
                url: '${cxt}/insuranceAction!selectDanHao.action',
                autoLoad: false,//自动加载
                stripeRows: true,//隔行变色
                rowHoverColor: true,//划过行变色
                displayBlankRows: false,//是否显示空白行, 默认值true
        ...
        function InsuranceNum() {
            $.post("${cxt}/insuranceAction!insuranceNum.action", function (data) {
        ...
        {
            if (ReceptionID != "") {
                $.getJSON("${cxt}/insuranceAction!selectInsurances.action?receptionID=" + ReceptionID, function (data) {
                    DanHaoData(data[0]);
                })
            } else {
                SeleceDanJu();
            }
            $.post("${cxt}/insuranceAction!clearInsurance.action");
        }
        /********************主页面---开始******************************/
        //删除数据
        function DelectInsurance() {
            ...
            if (InsuranceID != 0) {
                $.post("${cxt}/insuranceAction!delectInsurance.action?insuranceID=" + InsuranceID, function (data) {
                    if (data) {
                        layer.msg("删 除 成 功 ！");
                        SeleceDanJu();
                        Clear();
        ...
        function BavaMian() {
            ...
            if (MaintenanceNum == "") {
                ...
                $.post("${cxt}/insuranceAction!bavaInsurance.action", { pwInsurance: JSON.stringify(ArrInsurance), sysInsuranceDetail: JSON.stringify(ArrInsuranceDetail) }, function (data) {
                    if (data >0 ) {
                        SeleceDanJu()
        ...
        function TOAudit() {
            ...
            if (InsuranceID != "") {
                $("#ToAudit").val("true");
                $.post("${cxt}/insuranceAction!toAudit.action?insuranceID=" + InsuranceID, function (data) {
                    if (data > 0) {
        ...
        function TONotAudit() {
            ...
            $.getJSON("${cxt}/claimsetAction!selectBalance.action?businessNum=" + InsuranceNum, function (data) {
                if (data.state && data.states != "A") {
                    $.post("${cxt}/insuranceAction!toNotAudit.action?insuranceID=" + $("#InsuranceID").val(), function (data) {
                        if (data > 0) {
        ...
        function MianData(record) {
            Clear();
            $.getJSON("${cxt}/insuranceAction!selectReceptionDetail.action?receptionID=" + record.receptionId, function (data) {
        ...
        function DanHaoData(record) {
            ...
            console.log(record)
            $.getJSON("${cxt}/insuranceAction!selectInsuranceData.action?insuranceDetailID=" + InsuranceDetailID, function (data) {
                if (data != null) {
                    DanJuData(data[0]);
                } else {
                    MianData(record);
