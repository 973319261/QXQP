  ...
  <!--   @*按钮栏*@ -->
   ...
   <!--  @*表格*@ -->
    ...
    <script>
        ...
        {
            $.ajaxSettings.async=false;
            $.getJSON("${cxt}/collageAction!selectRestock.action?receptionID=" + $("#ReceptionID").val(), function (data) {
            console.log(data)
                data=data[0]
            ...
            $.getJSON("${cxt}/collageAction!arrlistToRestock.action", function (data) {
                console.log(data)
        ...
        function BaveRestock()
        {
            ...
            if ($("#RestockMan").val() == "") {
                ...
                $.getJSON("${cxt}/collageAction!judgingQuantity.action",{restockDetail: JSON.stringify(ArrRestockDetai)},function(data){
                    if (data.state) {
                        $.post("${cxt}/collageAction!updateListRestock.action", { pwRestock: JSON.stringify(ArrRestock), restockDetail: JSON.stringify(ArrRestockDetai) }, function (data) {
                            if (data>0) {
                                layer.msg("退 料 成 功 ！")
                            } else {
                                layer.msg("退 料 失 败 ！")
