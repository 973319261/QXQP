   ...
   <!--  @*配件基本资料添加模态框*@ -->
    ...
    <script>
        ...
        function MaintenanceNum() {
            $.post("${cxt}/customerAction!maintenanceNum.action", function (data) {
        ...
        function create() {
            ...
            appendOption("RepairID", "${cxt}/commonAction!selectAppendOption.action?type=SYS_Repair");//修理类别下拉框
            ...
            createSelect("PFittingsTypeID", "${cxt}/commonAction!selectAppendOption.action?type=SYS_FittingsType");//商品类别下拉框
            createSelect("PSystemUnitID", "${cxt}/commonAction!selectAppendOption.action?type=SYS_SystemUnit");//系统单位下拉框
        ...
        {
            $.getJSON("${cxt}/customerAction!arrlistToAppointment.action", function (data) {
                btnMainInsert(1);
                ...
                $("#Carder").val(text)
                $.post("${cxt}/customerAction!cleanArrlist.action");
            }
             layer.close(index);
          })     
        }
        ...
        function btnMainSava() {
            ...
            function Bave() {
                layer.confirm('是否保存当前信息？', {
                    ...
                    var index = layer.load(0); //0代表加载的风格，支持0-2
                    $.post("${cxt}/customerAction!updateListReceptione.action", {
                        ...
                        if (data > 0) {
                            layer.close(index);
                            $.post("${cxt}/appointmentAction!selectToMainten.action", { predateID: $("#PredateID").val(), maintenanceNum: $("#MaintenanceNum").text() });
        ...
        function ToDispatching()
        {
            var ReceptionID = $("#ReceptionID").val();
            if (ReceptionID != 0) {
                $.post("${cxt}/dispatchingAction!dispatching.action?receptionID=" + ReceptionID, function (data) {
        ...
        function ReceptionData(record) {
            var index = layer.load(0); //0代表加载的风格，支持0-2
            btnMainInsert(1);
            $.getJSON("${cxt}/customerAction!selectReceptionDetail.action?receptionID=" + record.receptionId, function (data) {
